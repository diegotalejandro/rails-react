class Reviser::PropuestasController < ApplicationController
  before_action :authenticate_reviser!
  before_action :set_propuestum, only: [:show, :edit, :update, :destroy]
  # GET /propuesta
  # GET /propuesta.json
  def index
    @propuestas = Propuestum.where reviser_id: current_reviser.id
  end
  # GET /propuesta/1
  ...
  def create
    ...
    @propuestum = Propuestum.new(propuestum_params)
      ...
      @propuestum.save
      @success=true
      redirect_to reviser_propuestas_path, notice: "propuesta creado exitosamente"
    else
  ...
  def update
    ...
    if Propuestum.exists?(params[:id])
      ...
      if @propuestum.update(propuestum_params)
        ...
        redirect_to reviser_propuestas_path, notice: "propuestum editado exitosamente"
      else
        flash.now[:alert] = "Ha ocurrido un error"
        render :edit
      end
    else
      redirect_to reviser_propuestas_path, alert: "propuestum no existe"
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to reviser_propuestas_path, notice: 'propuestum was successfully destroyed.' }
